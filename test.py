...
def test_get_task():
    r = requests.get('http://localhost:5000/v1/tasks/15')
...
def test_update_task():
    r = requests.put('http://localhost:5000/v1/tasks/15', json={"title": "My 1st Task", "is_completed": True})
    assert not r.content

def test_delete_task():
    r = requests.delete('http://localhost:5000/v1/tasks/15')
    assert not r.content
